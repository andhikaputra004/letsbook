    ...
    <nav class="navbar navbar-transparent navbar-absolute">
      <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse">
          ...
          <form class="navbar-form navbar-right" role="search" action="<?php echo site_url('transaksi/refund/search');?>" method="post">
                  ...
                  foreach ($list_transaksi as $transaksi)
                  {
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td><?php echo $transaksi->nama_event ?></td>
                        <td><?php echo $transaksi->nama_organisasi ?></td>
                        <td><?php echo $transaksi->waktu_transaksi ?></td>
                        <td class="td-actions text-right">
                        <?php                          
                          echo anchor(base_url().'transaksi/refund/detail/'.$transaksi->id_transaksi,'<button type="button" rel="tooltip" title="Read More" class="btn btn-info btn-simple btn-xs">
					...
					<div>
						<div class="col-md-3">
							<div class="form-group label-floating">
								...
								<input type="text" name="id_transaksi" class="form-control" disabled>
							</div>
						</div>
						<div class="col-md-3">
						  <div class="form-group label-floating">
								<label class="control-label">ID Pelanggan</label>
								<input type="text" name="id_pelanggan" class="form-control" disabled>
							</div>
						</div>
            <div class="col-md-36">
						  <div class="form-group label-floating">
								<label class="control-label">Nama Pelanggan</label>
								<input type="text" name="nama_pelanggan" class="form-control" disabled>
							</div>
						</div>
					</div>
...
function ViewData(id_pelanggan)
{
	var foto;
	$.ajax({
        url : "<?php echo site_url('Web_Transaksi_controller/ajax_edit')?>/" + id_transaksi,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
		      	$('[name="id_transaksi"]').text(data.id_transaksi);
            $('[name="id_pelanggan"]').text(data.id_pelanggan);
            $('[name="nama_pelanggan"]').text(data.nama_pelanggan);
            $('[name="id_event"]').text(data.id_event);
            $('[name="total_tagihan"]').text(data.total_tagihan);
            $('[name="status_tiket"]').text(data.status_tiket);
            $('[name="jumlah_tiket"]').text(data.jumlah_tiket);
            $('[name="waktu_transaksi"]').text(data.waktu_transaksi);
            $('[name="nama_event"]').text(data.nama_event);
            $('[name="status_event"]').text(data.status_event);
            $('[name="izin_refund"]').text(data.izin_refund);
            $('[name="gambar_poster"]').text(data.gambar_poster);
            $('[name="harga_tiket"]').text(data.harga_tiket);
            $('[name="biaya_pelayanan"]').text(data.biaya_pelayanan);
            $('[name="id_penyelenggara"]').text(data.id_penyelenggara);
            $('[name="nama_organisasi"]').text(data.nama_organisasi);
