...
<body>
    ...
    <nav class="navbar navbar-transparent navbar-absolute">
      <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse">
          ...
          <form class="navbar-form navbar-right" role="search" action="<?php echo site_url('eventselesai/search');?>" method="post">
          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header" data-background-color="purple">
                <h4 class="title">Event Selesai</h4>
              ...
              <div class="card-content table-responsive">
                <table class="table">
                  ...
                  <?php
                  $start = 0;
                  foreach ($list_event_selesai as $event)
                  {
                    ?>
                    <tbody>
                      <tr >
                        ...
                        <td><?php echo $event->status_pembayaran?></td>
                        <td><?php echo $event->tanggal_event?></td>
                        <td><?php echo "Rp ".number_format(($event->harga_tiket)*(($event->quota_peserta)-(($event->tiket_terjual)-($event->jumlah_tiket_refund)))) ?></td>
                        <td class="td-actions text-right" >
                          <?php
                          
                          echo anchor(base_url().'eventselesai/detail/'.$event->id_event,'<button type="button" rel="tooltip" title="Read More" class="btn btn-info btn-simple btn-xs">
                          <i class="material-icons">visibility</i>
                          </button>');
                          ?>
                          <button type="button" rel="tooltip" title="Upload Bukti Transfer" class="btn btn-primary" data-toggle="modal" data-target="#myModalEditPembayaran" onclick="EditDataPembayaran(<?php echo $event->id_event ?>)">
  ...
  <div class="modal fade" id="myModalEditPembayaran" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <form class="form" action="<?php echo site_url('eventselesai/update_transfer'); ?>" method="post" enctype="multipart/form-data">
      <div class="modal-dialog">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <input  type="text"  id="tagihan" name="tagihan" class="form-control" disabled>
					</div>

        <div class="input-group">
          <label>Bukti Transfer</label>
          <input type="file" class="btn btn-primary" name="bukti_bayar" id="bukti_bayar"/>
        </div>
...
<script type="text/javascript">
  ...
  function SaveId(id_event)
  {
    ...
    $.ajax({
      type: "GET",
      url: "<?php echo site_url('eventselesai/getid'); ?>",
      data: "id_event="+id_event
    }).done(function( msg ) {
    });
  }

  function EditDataPembayaran(id_event)
  {
      //Ajax Load data from ajax
      $.ajax({
          url : "<?php echo site_url('Web_EventSelesai_Controller/ajax_edit')?>/" + id_event,
          type: "GET",
          dataType: "JSON",
